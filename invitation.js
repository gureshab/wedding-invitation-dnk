// invitation.js — final gated script
(function () {
  "use strict";
  ...
  function revealOverlayAndInit() {
    ...
    const onEnd = (e) => {
      ...
      if (typeof window.initInvitation === "function") {
        ...
        // fallback safety
        setTimeout(() => {
          console.warn("initInvitation not found");
        }, 80);
      }
    };
  ...
  function bindOverlayControls() {
    ...
    overlay.addEventListener("click", function (e) {
      ...
      if (e.target === overlay || e.target.closest(".cover-card"))
        revealOverlayAndInit();
    });
  ...
  (function defineInit() {
    ...
    function getVisibleSlides() {
      ...
      if (onlyWedding) {
        const filtered = allSlides.filter((slide) => {
          ...
          const excluded =
            slide.getAttribute("data-exclude-on-only-wedding") === "true";
          const matchesSide = !showFor || showFor === guestSide;
          return matchesSide && !excluded;
        });
        return filtered.length
          ? filtered
          : allSlides.filter((s) => s.getAttribute("data-wedding") === "true");
      }
    ...
    function showSlide(n) {
      ...
      if (n > visible.length) currentSlideIndex = 1;
      if (n < 1) currentSlideIndex = visible.length;
      ...
      if (prevArrow && nextArrow) {
        ...
        prevArrow.style.pointerEvents =
          currentSlideIndex === 1 ? "none" : "auto";
        nextArrow.style.opacity =
          currentSlideIndex === visible.length ? "0" : "1";
        nextArrow.style.pointerEvents =
          currentSlideIndex === visible.length ? "none" : "auto";
      }
    }
    ...
    function createSparkle() {
      ...
      sparkle.style.cssText = `position:absolute;width:4px;height:4px;background:#d4af37;border-radius:50%;pointer-events:none;animation:sparkleFloat 2s ease-out forwards;left:${
        Math.random() * 100
      }%;top:${Math.random() * 100}%;box-shadow:0 0 10px #d4af37;`;
      container.appendChild(sparkle);
      setTimeout(() => sparkle.remove(), 2000);
    }
    function startSparkles() {
      if (sparkleTimer) clearInterval(sparkleTimer);
      sparkleTimer = setInterval(createSparkle, 500);
    }
    ...
    function createConfettiBurst() {
      ...
      for (let i = 0; i < 40; i++) {
        setTimeout(() => {
          ...
          c.style.cssText = `position:fixed;width:10px;height:10px;background:${
            colors[Math.floor(Math.random() * colors.length)]
          };left:${Math.random() * 100}vw;top:-10px;opacity:0.9;border-radius:${
            Math.random() > 0.5 ? "50%" : "0"
          };animation:confettiFall ${
            3 + Math.random() * 3
          }s linear forwards;z-index:9999;pointer-events:none;`;
